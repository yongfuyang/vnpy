...
class ThreeEmaStrategy(CtaTemplate):
    ...
    def onFiveBar(self, bar):
        ...
        # 判断买卖
        crossOver = self.fastMa[-1]>self.slowMa[-1] and self.fastMa[-2]<self.slowMa[-1] and self.longMa[-1]>0     # 金叉上穿
        crossBelow = self.fastMa[-1]<self.slowMa[-1] and self.fastMa[-2]>self.slowMa[-2] and self.longma[-1]<0    # 死叉下穿
