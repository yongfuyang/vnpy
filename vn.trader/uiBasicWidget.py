...
class NumCell(QtGui.QTableWidgetItem):
    ...
    def __lt__(self, other):
        if (isinstance(other, QtGui.QTableWidgetItem)):
            ...
            other_value, other_ok = other.data(Qt.DisplayRole).toInt()
...
########################################################################
class PositionDetailMonitor(BasicMonitor):
    """持仓明细监控"""

 # ----------------------------------------------------------------------
    def __init__(self, mainEngine, eventEngine, parent=None):
        """Constructor"""
        super(PositionDetailMonitor, self).__init__(mainEngine, eventEngine, parent)
        ...
        d['direction'] = {'chinese': u'方向', 'cellType': DirectionCell}
        d['position'] = {'chinese': u'持仓量', 'cellType': BasicCell}
        d['openprice'] = {'chinese': u'价格', 'cellType': BasicCell}
        d['opendate'] = {'chinese': u'开仓日期', 'cellType': BasicCell}
        d['gatewayName'] = {'chinese': u'接口', 'cellType': BasicCell}
        self.setHeaderDict(d)

        self.setDataKey('tradeid')
        self.setEventType(EVENT_POSITIONDETAIL)
        self.setFont(BASIC_FONT)
        self.setSaveData(True)
        self.initTable()
        self.registerEvent()
...
class TradingWidget(QtGui.QFrame):
    ...
    #-------------------------------------------------------------------------------------
    def getPrice(self, cell):
        """根据行情信息自动填写交易价格"""
        mkt = cell.data
        symbol = mkt.symbol

        if self.lineSymbol.text() == symbol :
            self.spinPrice.setValue(mkt.lastPrice)
